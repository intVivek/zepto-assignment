import {
  ...
  const isValueNotInArray = (value: string | number, array: Option[]) =>
    !array.some((item) => item.value === value);
  ...
  export default function MultiSelect({ placeholder = "Type Something...", options = [], values = [], onChange }: MultiSelectOptions) {
    ...
    const filteredOptions: Option[] = useMemo(() => {
      return options.filter((option) => {
        return (
          isValueNotInArray(option.value, values) &&
          doesLabelMatchInput(option.label, input)
        );
      });
    ...
    return (
      <div ref={parentRef} onKeyDown={handleKeyPress} className={style.select}>
        <div className={style.chipContainer}>
            ...
            return (
              <div
                className={`${style.chip} ${
                  highlight && i === values.length - 1 ? style.highlight : ""
                }`}
                key={i}
              >
                <Avatar className={style.avatar} icon={value.icon} name={value.label} />
                {value.label}
                <RxCross1 className={style.remove} onClick={() => removeItem(value)}
                />
              </div>
            );
          })}
          <div
            ref={inputRef}
            className={`${style.input} ${!values.length ? style.placeholder : ""}`}
            style={{ paddingLeft: values.length ? 0 : 8 }}
            ...
            <div className={style.optionsWindow}>
              {filteredOptions.length > 0 ? (
                filteredOptions.map((option, i) => {
                  return (
                    <div
                      className={style.option} onClick={() => handleItemClick(option)} key={i} >
                      <Avatar className={style.avatar} icon={option.icon} name={option.label} />
                      {option.label}
                    </div>
                  );
                })
